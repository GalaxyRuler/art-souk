...
export const artistAchievements = pgTable(
  'artist_achievements',
  {
    id: serial('id').primaryKey(),
    artistId: integer('artist_id').references(() => artists.id),
  ...
  (table) => [unique('artist_badge_unique').on(table.artistId, table.badgeId)]
);
...
export const badgeProgress = pgTable(
  ...
  (table) => [unique('artist_badge_progress_unique').on(table.artistId, table.badgeId)]
);
...
export const collectorWishlist = pgTable(
  'collector_wishlist',
  {
    id: serial('id').primaryKey(),
    userId: varchar('user_id')
  ...
  (table) => [unique('unique_user_artwork_wishlist').on(table.userId, table.artworkId)]
);
...
export const calendarIntegrations = pgTable(
  ...
  (table) => [unique('unique_user_provider').on(table.userId, table.provider)]
);

// Participant Lists
export const participantLists = pgTable(
  'participant_lists',
  {
    id: serial('id').primaryKey(),
    entityType: varchar('entity_type').notNull(), // 'workshop' or 'event'
  ...
  (table) => [unique('unique_entity_user').on(table.entityType, table.entityId, table.userId)]
);
...
export const waitlistEntries = pgTable(
  ...
  (table) => [unique('unique_waitlist_entry').on(table.entityType, table.entityId, table.userId)]
);
...
export const usersRelations = relations(users, ({ one, many }) => ({
  ...
  newsletterSubscription: one(newsletterSubscribers, {
    fields: [users.id],
    references: [newsletterSubscribers.userId],
  }),
...
export const artworksRelations = relations(artworks, ({ one, many }) => ({
  ...
  likes: many(likes),
  purchaseOrders: many(purchaseOrders),
  certificate: one(artworkCertificates, {
    fields: [artworks.id],
    references: [artworkCertificates.artworkId],
  }),
  wishlistItems: many(collectorWishlist),
  reviews: many(collectorReviews),
  auctionResults: many(auctionResults),
...
export const purchaseOrdersRelations = relations(purchaseOrders, ({ one }) => ({
  ...
  shippingTracking: one(shippingTracking, {
    ...
    references: [shippingTracking.orderId],
  }),
  installmentPlan: one(installmentPlans, {
    fields: [purchaseOrders.id],
    references: [installmentPlans.orderId],
  }),
  review: one(collectorReviews, {
    fields: [purchaseOrders.id],
    references: [collectorReviews.orderId],
  }),
}));
...
export const collectorReviewsRelations = relations(collectorReviews, ({ one }) => ({
  ...
  order: one(purchaseOrders, {
    fields: [collectorReviews.orderId],
    references: [purchaseOrders.id],
  }),
}));
...
export const emailNotificationQueueRelations = relations(
  emailNotificationQueue,
  ({ one, many }) => ({
    user: one(users, { fields: [emailNotificationQueue.recipientUserId], references: [users.id] }),
    template: one(emailTemplates, {
      fields: [emailNotificationQueue.templateCode],
      references: [emailTemplates.templateCode],
    }),
...
export const emailNotificationLogRelations = relations(emailNotificationLog, ({ one }) => ({
  ...
  queue: one(emailNotificationQueue, {
    fields: [emailNotificationLog.queueId],
    references: [emailNotificationQueue.id],
  }),
}));
...
export const artistAchievementsRelations = relations(artistAchievements, ({ one }) => ({
  ...
  badge: one(achievementBadges, {
    fields: [artistAchievements.badgeId],
...
export const insertAuctionResultSchema = createInsertSchema(auctionResults).omit({
  id: true,
  createdAt: true,
});
...
export const insertWorkshopEventReviewSchema = createInsertSchema(workshopEventReviews).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertEmailNotificationQueueSchema = createInsertSchema(emailNotificationQueue).omit({
  id: true,
  createdAt: true,
});
export const insertEmailNotificationLogSchema = createInsertSchema(emailNotificationLog).omit({
  id: true,
  sentAt: true,
});
...
export const insertArtistAchievementSchema = createInsertSchema(artistAchievements).omit({
  id: true,
  earnedAt: true,
});
export const insertArtistStatsSchema = createInsertSchema(artistStats).omit({
  id: true,
  updatedAt: true,
});
...
export const insertSchedulingConflictSchema = createInsertSchema(schedulingConflicts).omit({
  id: true,
  createdAt: true,
});
...
export const insertParticipantListSchema = createInsertSchema(participantLists).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLifecycleTransitionSchema = createInsertSchema(lifecycleTransitions).omit({
  id: true,
  transitionAt: true,
});
